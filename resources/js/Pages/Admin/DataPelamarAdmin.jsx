...
export default function DataPelamar({ datapelamar }) {
    ...
    const filteredData = datapelamar.filter((item) => {
        ...
        const matchesFilter = filterProdi === 'Semua' || item.prodi === filterProdi;
    ...
    const SortIcon = ({ columnKey }) => {
        if (sortConfig.key !== columnKey) {
            return (
                <svg className="w-3 h-3 text-gray-300 opacity-50 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        if (sortConfig.direction === 'asc') {
            return (
                <svg className="w-3 h-3 text-blue-600 font-bold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                >
                                    {uniqueProdi.map((prodi, index) => (
                                        <option key={index} value={prodi}>
                                            {prodi === 'Semua' ? 'Filter: Semua Prodi' : prodi}
                                        </option>
                                    ))}
                                </select>
